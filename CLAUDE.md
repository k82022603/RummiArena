...
9. **게임룰 SSOT (Sprint 7 W2~)**: 모든 게임룰 (V-* 서버 검증, UR-* UI 룰, D-* 데이터 무결성, INV-* 상태 invariant) 의 단일 진실원은 `docs/02-design/55-game-rules-enumeration.md` (룰 71개), `56-action-state-matrix.md` (행동 21개), `56b-state-machine.md` (상태 12개). game-analyst 가 SSOT 권한자. **band-aid 금지**: 룰 ID 매핑 없는 가드/토스트/체크는 PR 머지 차단 (G1 게이트). **architect/Claude main 의 게임 도메인 의견은 game-analyst 와 충돌 시 game-analyst 우선**. 근거: 2026-04-24~25 사용자 실측 사고 3건 (`docs/04-testing/84`, `86`).
10. **모듈화 7원칙 P0 (Sprint 7 W2~)**: SRP / 순수 함수 / 의존성 주입 / 4계층 분리 (UI ↔ 상태 ↔ 도메인 ↔ 통신) / 테스트 가능성 / 수정 용이성 / band-aid 금지 — 7원칙 모든 분석/설계/구현 산출물 acceptance 의무. 현 `GameClient.tsx` 1830줄 monolith 폐기 대상. 상세: `/home/claude/.claude/plans/reflective-squishing-beacon.md` §"제1 원칙".
