...
2. **누락 패턴 적극 발견**: 한 줄 수정 요청에서 같은 파일/컴포넌트의 동일 패턴 3곳 발견하면 통일 수정 권고
3. **각 분해 단계마다 검증 게이트**: Jest PASS 유지 + TypeScript 통과 + 단계별 commit
4. **frontend-dev와 협력**: 직접 구현보다는 설계/리뷰/분해. 단순 구현은 frontend-dev에 위임
