...
> **기준 커밋**: `40a2732` (main, 2026-03-30 데일리 마감)
> **최종 갱신**: 2026-03-30 오후 세션 (사용자 테스트 버그 추가)
> **작성자**: PM (애벌레)
...
| BL-S4-005 | 비용 추적 Redis 연동 (quota:daily Hash) | P1 | Node Dev | 3 | BL-S4-001 | **완료** |
| BL-S4-006 | 모델별 성능 비교 메트릭 수집 (응답 시간, 토큰, 유효 수 비율) | P1 | Node Dev / QA | 3 | BL-S4-004 | **완료** |
| BL-S4-007 | AI 호출 로그/메트릭 수집 구현 (admin /stats/ai 실 데이터화) | P1 | Node Dev / Go Dev | 5 | BL-S4-001 | 미완료 |

**소계**: 41 SP (Sprint 4 원 계획 30 SP + 추가 식별 11 SP) / **완료 6 SP** (BL-S4-005, BL-S4-006)

---

## 1-1. Sprint 4 설계 구현 완료 항목 (2026-03-30 오전 세션)

`docs/02-design/12-player-lifecycle-design.md` 4개 기능 구현 + 테스트 완료.

| ID | 제목 | SP | 설계 문서 | 구현 문서 | 상태 |
|----|------|----|-----------|-----------|------|
| BL-S4-LC-001 | 게임 중 퇴장/기권 (Grace 60초, FORFEITED, advanceTurn 건너뛰기) | 7 | 12-player-lifecycle-design 1절 | 12-lifecycle-implementation-notes 2절 | **완료** |
| BL-S4-LC-002 | 중복 방 참가 제한 (인메모리 역인덱스, CreateRoom/JoinRoom 검증) | 3 | 12-player-lifecycle-design 2절 | 12-lifecycle-implementation-notes 2.4절 | **완료** |
| BL-S4-LC-003 | 교착 처리 규칙 수정 (드로우 소진 패스 모드, 전원 패스 종료) | 5 | 12-player-lifecycle-design 3절 | 12-lifecycle-implementation-notes 2.3절 | **완료** |
| BL-S4-LC-004 | beforeunload 경고 (useGameLeaveGuard 훅, PLAYING 상태) | 2 | 12-player-lifecycle-design 4절 | 12-lifecycle-implementation-notes 4.1절 | **완료** |
...
| 계층 | 전체 | PASS | 비고 |
|------|------|------|------|
| Go 유닛 | 346 | 329 PASS + 17 SKIP | 신규 15개 (advanceTurn 6, ELO 기권 3, 역인덱스 4, 교착 2) |
| Node 유닛 | 318 | 318 PASS | 신규 45개 (비용 추적 35, 메트릭 10) |
| E2E | 153 | 153 PASS | 신규 22개 (beforeunload 8, 교착 UI 4, 퇴장/기권 UI 10) |
| **총계** | **817** | **800 PASS + 17 SKIP** | 상세: `docs/04-testing/22-lifecycle-implementation-test-report.md` |

---

## 1-2. 사용자 테스트 발견 버그 (2026-03-30 오후 세션)

애벌레가 직접 게임 플레이(25장 스크린샷)하며 발견한 버그. 커밋 f5495fa로 5건 수정 완료, 5건 신규 미수정.

### 수정 완료 (커밋 f5495fa, 배포 완료)

| ID | 제목 | 심각도 | 수정 파일 | 상태 |
|----|------|--------|-----------|------|
| BUG-WS-001 | WS 끊김 반복 ("재연결 시도 중" 배너 지속) | CRITICAL | `ws_connection.go` (ping/pong heartbeat 25s/60s) | **배포 완료** |
| BUG-AI-002 | AI DisplayName "GPT 0" → 캐릭터명 미표시 | HIGH | `room_service.go` (formatAIDisplayName) | **배포 완료** |
| BUG-UI-TILE | 타일 a/b 세트 구분 불가 | HIGH | `Tile.tsx` (b세트 도트 표시) | **배포 완료** |
| BUG-UI-DEF | 동일색상 그룹 사전 차단 없음 | HIGH | `GameBoard.tsx` (중복색 경고) | **배포 완료** |
| BUG-UI-30PT | 30점 최초 등록 점수 실시간 미표시 | MEDIUM | `GameClient.tsx` (pendingPlacementScore) | **배포 완료** |

### 미수정 — 이번 세션 수정 대상

| ID | 제목 | 심각도 | 담당 | SP | 상태 |
|----|------|--------|------|----|------|
| BUG-AI-001 | AI 12턴 연속 드로우만 (전략 고장, "등록 전" 22턴 유지) | CRITICAL | Go Dev / Node Dev | 5 | **미수정** |
| BUG-UI-005 | 런/그룹 오분류 (같은 숫자 배치 → "런(미확정)" 표시) | HIGH | Frontend Dev | 2 | **미수정** |
| BUG-UI-006 | 배치 실패 시 피드백 없음 (타일 무언 반환) | HIGH | Frontend Dev | 2 | **미수정** |
| BUG-UI-007 | 이름 간헐적 잘림 ("내 차례" 배지 공간 부족) | MEDIUM | Frontend Dev | 1 | **미수정** |
| BUG-AI-003 | GPT "등록 전" 22턴 유지 (BUG-AI-001 종속) | MEDIUM | Go Dev / Node Dev | - | **미수정** |
...
| BL-P0-003 | Git 히스토리 시크릿 정리 (bfg/filter-branch) | SEC-002 | **P0** | DevOps | 2 | BL-P0-002 | **완료** |
| BL-P0-004 | OpenAI/Claude/DeepSeek API 키 로테이션 | SEC-005 | **P0** | 애벌레 | 1 | BL-P0-002 | 미착수 |
| BL-P0-005 | K8s Pod securityContext 추가 (4+3개 Deployment) | SEC-003 | **P0** | DevOps | 2 | 없음 | **완료** |

**소계**: 9 SP / **완료 4 SP** (BL-P0-003, BL-P0-005)
...
| BL-P1-003 | 심리전 프롬프트 버그 수정 (psychWarfarePrompt 합산 누락) | W-CODE-01 | P1 | Node Dev | 1 | 없음 | **완료** (이미 반영 확인) |
...
| BL-P2-007 | Ollama num_predict 512 상향 + stop 토큰 정리 | W-CODE-10 | P2 | Node Dev | 1 | 없음 | **완료** |
...
```mermaid
flowchart TB
    subgraph S4LC["Sprint 4 설계 구현 (17 SP) -- 완료"]
        direction LR
        A0["BL-S4-LC-001~004\n퇴장/기권, 중복방, 교착, beforeunload"]
    end

    subgraph P0["P0 긴급 (5/9 SP 잔여)"]
        direction LR
        A1["BL-P0-001~002,004\n보안 CRITICAL 3건 잔여"]
    end

    subgraph S4R["Sprint 4 잔여 (35/41 SP 잔여)"]
        direction LR
        A2["BL-S4-001~004,007\nLLM 실 연동 + E2E"]
    end
...
| 카테고리 | 항목 수 | 합계 SP | 완료 SP | 잔여 SP | 목표 시점 |
|----------|---------|---------|---------|---------|-----------|
| Sprint 4 잔여 | 7 | 41 | 6 | 35 | Sprint 4 내 완료 |
| Sprint 4 설계 구현 | 4 | 17 | **17** | 0 | **완료** |
| P0 긴급 | 5 | 9 | 4 | 5 | 즉시 (Sprint 4 병행) |
| P1 보안+안정 | 10 | 24 | 1 | 23 | Sprint 5 초반 (1주차) |
| P2 테스트+관측 | 10 | 39 | 1 | 38 | Sprint 5 중후반 (2주차) |
| P3 UX+확장 | 15 | 55 | 0 | 55 | Sprint 5~6 |
| 기술 부채 | 31 | 63 | 0 | 63 | 점진적 해소 |
| **총계** | **82** | **248** | **29** | **219** | - |
...
| CR-01 | LLM API 비용 폭증 | 높음 | 심각 | **심각** | **부분 완화** (비용 추적 + 429 Guard 구현) | BL-S4-005 (**완료**) |
| NR-01 | Admin API 무인가 접근 | 높음 | 심각 | **심각** | **신규** | BL-P0-001 |
| NR-02 | Git 히스토리 시크릿 노출 | 높음 | 심각 | **심각** | **부분 완화** (히스토리 정리 완료, 로테이션 잔여) | BL-P0-002, BL-P0-003 (**완료**) |
