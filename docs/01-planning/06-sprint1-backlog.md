...
| S1-02 | 타일 도메인 모델 | 3 | P0 | S1-01 | 2.1.1 | In Progress (80%) |
| S1-03 | TilePool 생성/셔플/분배 | 2 | P0 | S1-02 | 2.1.2 | To Do |
| S1-04 | 그룹(Group) 유효성 검증 | 3 | P0 | S1-02 | 2.1.3 | In Progress (기본 구현 완료, 엣지케이스 보강 필요) |
| S1-05 | 런(Run) 유효성 검증 | 3 | P0 | S1-02 | 2.1.4 | In Progress (기본 구현 완료, 조커 로직 보강 필요) |
| S1-06 | 세트 유효성 통합 검증 (Validator) | 5 | P0 | S1-04, S1-05 | 2.1.3, 2.1.5, 2.1.6 | In Progress (코어 완료, 테스트 보강 필요) |
| S1-07 | 턴 관리 및 승리 판정 | 5 | P1 | S1-06, S1-03 | 2.1.7, 2.1.8, 2.1.9 | To Do (TODO 스텁만) |
| S1-08 | 기본 REST API | 5 | P1 | S1-01 | 2.2.2, 2.2.6 | In Progress (/health, /ready만 완료) |
| S1-09 | PostgreSQL 스키마 마이그레이션 | 3 | P1 | S1-01 | 2.2.5 | **Done** (03-13, K8s에서 10 테이블 마이그레이션 완료) |
| S1-10 | Redis 연동 | 3 | P1 | S1-01 | 2.2.4 | In Progress (기본 CRUD 구현, 키 구조 확장 필요) |
| S1-11 | Docker 이미지 빌드 | 2 | P2 | S1-01, S1-08 | 2.2.8 | **Done** (03-13, 4개 Dockerfile + K8s Helm 배포 완료) |
| S1-12 | 게임 엔진 단위 테스트 | 5 | P1 | S1-04~S1-07 | 2.1.10 | In Progress (69개 테스트, 커버리지 96.5%, V-08~V-12 미커버) |
| | **합계** | **42** | | | | |

> **진행 현황 (2026-03-13 기준)**: S1-01, S1-09, S1-11 완료 (8 SP). 잔여 34 SP 중 엔진 라인(S1-02~S1-07)이 크리티컬 패스. engine 테스트 69개/96.5% 커버리지로 S1-12도 상당 부분 진행됨. K8s 전환 완료로 S1-11(Docker)이 당초 예상보다 초과 달성 (Helm chart + K8s 배포까지 포함).
...
| R5 | Sprint 0 인프라 미완료 항목(K8s, ArgoCD)과 병행 부담 | 중간 | ~~중간~~ **해소** | ~~Sprint 1에서는 Docker Compose로 개발~~ K8s 전환 완료 (03-13). Helm chart 5개 + rummikub namespace 배포 |
| R6 | 42 SP가 1인 2주에 과부하 | 높음 | ~~중간~~ **낮음** | 수정 계획(07-sprint1-revised-plan.md)으로 28 SP 조정. S1-01/S1-09/S1-11 완료로 잔여 부담 감소 |
| R7 | K8s ResourceQuota init container 누락 | 중간 | 해결 | Helm chart 템플릿에 init container resource 기본값 반영 (ISS-02) |
| R8 | 로컬 .env와 K8s ConfigMap 환경변수 동기화 누락 | 중간 | 중간 | 환경변수 동기화 체크리스트 작성 필요 (ISS-03) |
...
| Docker 이미지 + K8s 배포 | ~~Helm Chart 작성 시 이미지 참조~~ **Helm chart 5개 + K8s 배포 완료 (03-13). Sprint 2에서는 ArgoCD 연동 + CI/CD 파이프라인 구성** |
