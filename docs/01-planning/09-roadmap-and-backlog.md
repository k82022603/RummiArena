...
> **최종 업데이트**: 2026-03-22
> **현재 위치**: Sprint 2 완료 (50/50 SP = 100%) → Sprint 3 계획 중 (2026-03-29 시작 예정)
...
| Phase 4 ELO 랭킹 (#25~#27) | ✅ 완료 (2026-03-22, Sprint 2 내 선행) |
| Sprint 3 계획 (#28~#31) | 🔶 계획 중 (2026-03-29 시작 예정) |
...
### [#25] ELO DB + Game Engine 연동 ✅ 완료 (8 SP, 2026-03-22)

- **서비스**: game-server (Go)
- **완료 산출물**:
  - EloRating GORM 모델 + EloHistory 감사 로그 테이블
  - Pairwise ELO 계산 엔진 (K-Factor 32/40/24, 6개 티어, 21개 단위 테스트)
  - EloRepository 인터페이스 + PostgreSQL 구현
  - GET /api/rankings 4개 엔드포인트 (10개 핸들러 테스트)
  - GAME_OVER → updateElo() goroutine 비동기 처리
  - Redis Sorted Set (`ranking:global`, `ranking:tier:{tier}`)

### [#26] Admin ELO 통계 패널 ✅ 완료 (5 SP, 2026-03-22)

- **서비스**: admin (Next.js)
- **완료 산출물**:
  - EloRankingPanel: recharts PieChart (티어 분포) + 리더보드 + 요약 카드
  - /rankings Server Component (revalidate=30)
  - USE_MOCK 패턴 (api.ts + mock-data.ts)
  - Sidebar ELO 랭킹 메뉴 추가

### [#27] Frontend 랭킹 페이지 ✅ 완료 (8 SP, 2026-03-22)

- **서비스**: frontend (Next.js)
- **완료 산출물**:
  - /rankings: 티어 필터 탭 + 리더보드 테이블 + 페이지네이션 (20개/페이지)
  - /rankings/[userId]: 개인 ELO 프로필 + SVG LineChart (recharts 미사용)
...
| Sprint | 기간 | 목표 | 핵심 작업 |
|--------|------|------|-----------|
| **Sprint 2** | **03-22 (조기 완료)** | **AI 캐릭터 & ELO** | **#20~#27 전량 완료 (50 SP, 100%)** |
| **Sprint 3** | **03-29 ~ 04-11** | **프론트엔드 완성** | **Google OAuth, 게임 보드 완성, WebSocket 재연결, gemma3:4b 최적화** |
| Sprint 4 | 04-12 ~ 04-25 | AI Adapter 4종 | OpenAI/Claude/DeepSeek/Ollama 연동 |
...
| admin (Next.js) | ~60% | 대시보드 + 실 API 연동 + ELO 통계 패널 (recharts PieChart + 리더보드) |
