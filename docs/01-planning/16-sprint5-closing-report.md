...
Sprint 5는 RummiArena 프로젝트의 **DevSecOps 자동화 확립** 스프린트이다. "코드 품질 게이트를 자동화하고, 서비스 메시로 내부 통신을 보호한다"라는 미션 하에, GitLab CI/CD 17/17 ALL GREEN, SonarQube + Trivy 보안 게이트, 보안 9건 해결, AI 대전 Round 2~5(3모델 다회 대전 12회 + 토큰 효율성 분석), 테스트 1,528건 달성을 이루었다.
...
#### 3.2.2 Round 5: v2 다회 대전 (04-10~11)

Sprint 5 마지막 이틀간 3모델 multirun 3회 + BUG-GS-005 수정 후 검증 대전 3모델 각 1회를 실행하여 **총 12회 대전** 데이터를 확보했다.

| 구분 | 모델 | Run 1 | Run 2 | Run 3 | 검증 | 평균 |
|------|------|-------|-------|-------|------|------|
| multirun | DeepSeek | 20.5% | 25.6% | 30.8% | — | 25.6% |
| multirun | GPT | 33.3% | 30.8% | 21.9%* | — | 28.7% |
| multirun | Claude | 28.2% | 19.0%** | 33.3% | — | 26.8% |
| 검증 | DeepSeek | — | — | — | **33.3%** | — |
| 검증 | GPT | — | — | — | 28.2% | — |
| 검증 | Claude | — | — | — | 25.6% | — |
...
**검증 대전**: BUG-GS-005 수정 후 3모델 모두 **80턴 완주, Fallback 0, WS_TIMEOUT 0**

#### 3.2.3 최종 모델 순위 (v2, 전체 데이터 기반)

```
GPT-5-mini (29.5%) ≈ Claude Sonnet 4 (28.5%) ≈ DeepSeek Reasoner (25.6%) >> Ollama (0%)
```

> 다회 대전으로 모델 간 차이가 5%p 이내로 수렴 — "추론 모델이면 비슷하다"

#### 3.2.4 DeepSeek 성장 추이
...
6. **토큰 효율성 분석**: DeepSeek 37 tok/s vs GPT 69 tok/s vs Claude 66 tok/s. 상세: [47-reasoning-model-deep-analysis.md](../04-testing/47-reasoning-model-deep-analysis.md)
7. **Claude 28턴 침묵 후 폭발**: 초반 Place 0%, T30에서 10타일 한번에 배치 — 고유 전략 패턴.
...
| 최고 Place Rate | **33.3%** (DeepSeek/GPT/Claude 공동, 검증 대전 DeepSeek) |
| 최저 비용 모델 | DeepSeek Reasoner ($0.001/턴, Place당 $0.003) |
| 프롬프트 버전 | v1 -> v2 (v3 초안 완료) |
| 모델 비교 | 4종 (GPT/Claude/DeepSeek/Ollama) |
| 토큰 효율성 분석 | 3모델 tok/s, Place/Draw 패턴, 구간별 전략 (47번 보고서) |
...
- **핵심 초과 달성**: AI 대전 12회 다회 실행 + 토큰 효율성 분석 + BUG-GS-005 수정
