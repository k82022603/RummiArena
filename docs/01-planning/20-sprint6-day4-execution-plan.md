...
4. PromptRegistry 로그에서 v4 load 확인
   ...
   ```
5. **GPT / Ollama는 v2 유지** (v4 부적합 판정, SP5 문서 참조 + §3bis line 143~149 — "v3 유지" 는 과거 잘못된 표현이고 Round 4~5 가 실제로 v2 였음이 SP3 에서 확정됨. 따라서 "유지" 의 실제 대상은 v2)
...
**구성**:
- GPT-5-mini × 1 게임 (80턴, **v2 유지** — §3bis 참조. 원본 "v3 유지" 는 SP3 이전 과거 표현)
...
| 9 | AI Engineer | Phase 3 대조군 (GPT × 1, **v2 유지** — §Phase 3 "Day 4 실행 중 업데이트" 참조. GPT 는 v3 를 한 번도 돌린 적이 없음. 원안의 "v3 유지" 는 SP3 "Round 4~5 가 v2 였다" 발견 이전 텍스트이며 line 121 에서 확정 덮어씀) | Day 4 저녁 (~16:00~17:00) | #6 완료 |
| 10 | AI Engineer | Round 6 결과 리포트 `docs/03-development/22` (**v2 (Round 4/5) vs v4 (Round 6) 실측 비교**. "v3 효과" 는 여전히 미측정) | Day 4 저녁 (~17:00~18:00) | #9 완료 |
