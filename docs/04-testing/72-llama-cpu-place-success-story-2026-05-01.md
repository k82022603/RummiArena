...
LLM이 세상을 바꾼다고 한다. 추론하고, 계획하고, 코드를 짜고, 글을 쓴다. 실제로 그런다. 하지만 루미큐브 손패 14장에서 30점 조합을 찾는 것은 못 한다. 적어도 작은 모델은.

이것이 LLM의 한계를 보여주는 이야기인가? 오히려 그 반대다. LLM이 무엇을 잘하는지, 무엇을 알고리즘에 맡겨야 하는지를 이해하는 사람만이 두 도구를 제대로 쓸 수 있다.

v8은 그 이해의 산물이다. 모델에게 어울리지 않는 짐을 내려놓고, 모델이 잘하는 것 — 주어진 JSON을 그대로 출력하는 것 — 만 남겼다. 그랬더니 타일이 테이블에 올라갔다.
