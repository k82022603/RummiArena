...
### 모델별 대전 결과 — Round 1 (프롬프트 강화 전)

| 모델 | 유형 | Place | 배치 타일 | Place Rate | 결과 | 비고 |
|------|------|-------|----------|-----------|------|------|
| gpt-5-mini (shark) | 추론 | 5회 | 17장 | 12% | STALEMATE | timeout 60s로 재시도 多 |
| Claude Opus (shark) | 비추론 | 2회 | 10장 | 5% | STALEMATE | thinking 없어 조합 탐색 한계 |
| DeepSeek reasoner (calculator) | 추론 | 0회 | 0장 | 0% | WS 끊김 | timeout 30s→실패→212s 초과 |

### Game Engine 거부 108건 분석

| 에러 코드 | 설명 | Claude | DeepSeek | OpenAI | 합계 |
|----------|------|--------|----------|--------|------|
| ERR_GROUP_COLOR_DUP | 그룹 내 같은 색 중복 | **35** | 1 | 1 | 37 |
| ERR_SET_SIZE | 3장 미만 | 6 | **23** | 8 | 37 |
| ERR_GROUP_NUMBER | 그룹 내 숫자 불일치 | **29** | 0 | 0 | 29 |
| 기타 | 알 수 없음 | 1 | 3 | 1 | 5 |
| **합계** | | **71** | **27** | **10** | **108** |

### 대응 1: 게임 룰 프롬프트 전면 강화

108건 에러 분석 결과 → 전체 게임 룰 프롬프트 강화:
1. 그룹 규칙: 올바른 예시 2개 + 잘못된 예시 3개 (에러코드 명시)
2. 런 규칙: 올바른 예시 2개 + 잘못된 예시 3개 (색상 혼합, 비연속, 순환)
3. 크기 규칙: "3장 미만 무효" 강조 (ERR_SET_SIZE)
4. 전략 분석 절차 + 응답 예시 4개 추가 (draw, 단일런, 복수런, 기존 테이블 확장)

### 대응 2: 추론 모델 타임아웃 체인 수정

| 어댑터 | 수정 전 | 수정 후 | 근거 |
|--------|--------|--------|------|
| OpenAI (gpt-5-mini) | 60s | **120s** | 실측 51s/턴, 간헐 60s 초과 |
| DeepSeek (reasoner) | 60s | **150s** | 실측 92~128s/턴, API 자체가 느림 |
| Claude (Sonnet 4 thinking) | 30s | **120s** | extended thinking 64s/턴 |

### 대응 3: Claude Opus → Sonnet 4 + Extended Thinking 전환

비추론 모델(Opus)은 타일 조합 탐색(combinatorial search)에 약함.
Extended thinking 활성화로 reasoning token을 통한 사고 과정을 부여.

- `CLAUDE_DEFAULT_MODEL`: `claude-opus-4` → `claude-sonnet-4-20250514`
- `CLAUDE_EXTENDED_THINKING`: `true` (budget_tokens: 10000)
- API 비용도 Opus 대비 저렴

### 모델별 대전 결과 — Round 2 (프롬프트 강화 + 타임아웃 수정 후)

| 모델 | 유형 | Place | 배치 타일 | Draw | FB | Rate | 턴 | 시간 | 결과 |
|------|------|-------|----------|------|-----|------|----|------|------|
| **gpt-5-mini** (shark) | 추론 | **11** | **27** | 28 | 0 | **28%** | 80 | 1876s | MAX_TURNS |
| **Claude Sonnet 4** (shark, thinking) | 추론 | **9** | **29** | 30 | 0 | **23%** | 80 | 2076s | MAX_TURNS |
| **DeepSeek Reasoner** (calculator) | 추론 | **2** | **14** | 37 | 0 | **5%** | 80 | 1995s | MAX_TURNS |

### Round 1 → Round 2 개선 비교

| 모델 | R1 Rate | R2 Rate | 개선율 | 핵심 변경 |
|------|---------|---------|--------|----------|
| gpt-5-mini | 12% | **28%** | 2.3x | timeout 60s→120s, fallback 10건→0건 |
| Claude | 5% | **23%** | 4.6x | Opus→Sonnet 4 + extended thinking |
| DeepSeek | 0% (WS끊김) | **5%** | ∞ | timeout 60s→150s, WS 안정화 |

### 주요 발견

1. **추론 모델이 압도적** — gpt-5-mini(28%), Sonnet 4+thinking(23%) vs 비추론 Opus(5%)
2. **타임아웃이 성능의 전부** — 충분한 시간을 주면 모든 모델이 배치 가능. 시간 부족→fallback
3. **전 모델 Fallback 0건** — 타임아웃 수정으로 불필요한 강제 드로우 완전 제거
4. **DeepSeek 11타일 폭탄** — T20에서 한 번에 11타일 배치, reasoning의 deep search 증거
5. **비용** — gpt-5-mini $0.025/턴, Claude Sonnet 4 $0.074/턴, DeepSeek $0.001/턴

### 후속 조치
- [x] AI_ADAPTER_URL Helm values 추가 (커밋 88600c5)
- [x] gpt-5-mini 추론 타임아웃 60초 보장 (커밋 88600c5)
- [x] 프롬프트 전면 강화 (그룹/런 규칙, 예시, 전략 절차)
- [x] 추론 모델 타임아웃 체인 수정 (OpenAI 120s, DeepSeek 150s, Claude 120s)
- [x] Claude Sonnet 4 + Extended Thinking 전환
- [x] 전 모델 AI 대전 재테스트 (Round 2)
- [ ] DeepSeek Reasoner place rate 개선 (5% → 목표 15%+)
