...
## 7. 추가 실행 결과 및 재분석 (2026-04-08 오후)

### 7.1 2차 실행: RATE_LIMIT_LOW_MAX=60

환경변수 외부화 후 `RATE_LIMIT_LOW_MAX=60`으로 K8s ConfigMap 반영, game-server 재배포.
...
**판단**: 60 req/min은 E2E 390건의 누적 부하에 부족. 전체 스위트가 동일 유저(dev login)로 실행되며, cleanup/네비게이션 등이 "low" 엔드포인트를 누적 소비.

### 7.2 3차 실행: RATE_LIMIT_LOW_MAX=200

`RATE_LIMIT_LOW_MAX=200`으로 상향 후 재배포, E2E 재실행.

| 결과 | 건수 | 비고 |
|------|------|------|
| PASS | 32 | |
| FAIL | 8 | TC-AB-002~010 (TC-AB-008 제외) |
| 실행 건수 | 40/390 | Playwright 프로세스 조기 종료 |

**핵심 발견: 서버는 429를 반환하지 않았다.**

| 검증 항목 | 결과 |
|----------|------|
| Redis 카운터 (`ratelimit:*:low`) | 4~17 / 200 한도 (한참 이내) |
| game-server 로그 `grep 429` | **0건** — 서버가 429를 보내지 않음 |
| 프론트엔드 toast | "요청이 너무 빨랐습니다. **5초** 후 다시 시도합니다" 표시 |
| `DEFAULT_RETRY_AFTER_SEC` | 5초 — Retry-After 헤더 없을 때 기본값 |

### 7.3 재분석: Rate Limit이 아닌 다른 원인

서버가 429를 보내지 않는데 프론트엔드 toast가 표시되는 것은 다음 중 하나를 의미한다:

1. **프론트엔드 Zustand 스토어 잔존**: 이전 테스트(1차/2차 실행)에서 설정된 `rateLimitStore` 상태가 Playwright 브라우저 컨텍스트에 남아 있을 가능성
2. **`createAIBattle` 헬퍼의 cleanup 실패**: TC-AB-001에서 생성한 게임이 TC-AB-002 시작 전에 정리되지 않아 409 ALREADY_IN_ROOM 또는 기타 오류 발생, 이를 프론트엔드가 rate limit으로 오인
3. **Next.js rewrite proxy 계층**: 프론트엔드 Pod → game-server Pod 프록시 과정에서 별도의 rate limit 또는 오류 발생

**결정적 증거**:
- TC-AB-001 PASS (방 생성 성공, 게임 진입)
- TC-AB-002~007 FAIL (각 16초 — `waitForURL` 15초 타임아웃)
- TC-AB-008 PASS (방 생성 불필요한 UI 확인 테스트)
- TC-AB-009~010 FAIL (방 생성 필요)

→ **방 생성을 시도하는 테스트만 실패**, UI 확인만 하는 테스트는 통과.

### 7.4 다음 조치 (Sprint 5 W2 Day 4)

| 순서 | 항목 | 담당 |
|------|------|------|
| 1 | `createAIBattle` 실패 시 error-context 상세 캡처 (HTTP status code 로깅) | Frontend Dev |
| 2 | E2E 실행 중 Redis rate limit 키 실시간 모니터링 | QA |
| 3 | `cleanupViaPage` 동작 검증 — 이전 게임 세션이 정상 종료되는지 확인 | Frontend Dev |
| 4 | Playwright 브라우저 컨텍스트 간 Zustand 스토어 초기화 여부 확인 | Frontend Dev |
| 5 | game-server 429 응답 로깅 추가 (현재 로그 없음) | Go Dev |

### 7.5 인사이트

1. **보안 기능과 테스트는 같이 설계해야 한다** — Rate Limit 구현 시 E2E 영향도 사전 분석 누락이 근본 원인
2. **하드코딩은 언젠가 발목을 잡는다** — 설정은 외부화가 기본
3. **실패 패턴 분석이 진짜 가치다** — 47건 실패 중 44건이 동일 원인. 건수가 아니라 패턴을 읽어야 한다
4. **단일 테스트가 아니라 전체 스위트의 누적 부하 기준** — 60으로도 부족했던 이유는 390건의 누적 API 호출
5. **관찰 가능해야 판단할 수 있다** — `| tail -40` 파이프로 30분간 진행 상황 확인 불가. 실시간 관찰 없이는 판단도 늦어진다
6. **설정값 문제라고 단정하지 말 것** — 200으로 올려도 동일 패턴 실패. Redis 카운터와 서버 로그를 교차 검증하니 서버는 429를 보내지 않았다. 프론트엔드 상태 잔존 또는 E2E 구조 문제일 가능성이 높다
