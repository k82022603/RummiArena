...
interface TurnStatusVM {
  ...
  // 라운드 (1-based)
  roundIndex: number;
  // 2인: 최근 2라운드 도트 (prevRound + currentRound)
  // 3인+: 현재 라운드 도트만
  rounds: Array<{
    roundIndex: number;
    turns: Array<'done' | 'current' | 'pending'>;
  }>;
  turnsCompleted: number;      // 현재 라운드 기준
  turnsTotal: number;          // = playerCount
}
```

### 3.1 라운드 도트 규칙 (2026-04-28 결정)

| 플레이어 수 | 표시 범위 | NEXT 블록 | 비고 |
|---|---|---|---|
| **2인** | 최근 2라운드 (예: R6 2도트 + R7 2도트 = 4도트) | **숨김** — 라운드 도트가 넓어진 자리 활용 | 라운드 사이에 세로 구분선 1px `#1e2532` |
| **3인** | 현재 라운드만 (3도트) | 표시 | — |
| **4인** | 현재 라운드만 (4도트) | 표시 | — |

서버 이벤트 매핑 (참고용):
- `turn:start` → `current`, `next`, `roundIndex`, `rounds` 갱신
- `turn:tick` → `remainingSec`만 갱신
- `turn:end` → 해당 라운드의 `turns[i]`를 `'done'`으로
...
#### 2인 대전 특수 규칙 (2026-04-28 결정)

2인일 때 도트 2개만 표시하면 휑하므로, **최근 2라운드를 묶어** 4도트로 표시한다.
...
- 라운드 사이에 세로 구분선: `1px solid #1e2532`, 높이 = 도트 높이(14px)
- 라운드 라벨을 `"ROUND 6–7"` 형식으로 변경
- 턴 카운터는 현재 라운드 기준: `"1 / 2 턴"`
- 이전 라운드가 없으면 (R1) 현재 라운드만 표시 (2도트)

#### NEXT 블록 표시 규칙

| 플레이어 수 | NEXT 블록 |
|---|---|
| **2인** | **숨김** — 상대가 항상 다음이므로 불필요. 빈 자리를 라운드 도트 확장에 활용 |
| **3인+** | **항상 표시** — 반응형 숨김만 적용 (§4.4) |

### 4.4 반응형

| 폭 | 3인+ | 2인 |
|---|---|---|
| ≥ 1024px | 전체 (Timer · Now · Next · Round) | Timer · Now · Round (2라운드 확장) |
| 768~1023px | Timer · Now · Round (Next 숨김) | Timer · Now · Round |
| < 768px | Timer + Now만, Round는 헤더로 이동 | Timer + Now만 |
...
- **Round의 "총 턴 수" 정의**: 한 라운드 = 모든 플레이어가 1턴씩 (= n명) 으로 가도 되는지, 아니면 별도 정의가 있는지
- **"NEXT" 표시의 끄기 옵션**: 다음 차례 정보가 필요 없다면 1024px 이상에서도 숨길 수 있는 설정을 둘지
