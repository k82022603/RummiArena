...
| `ollama` (qwen2.5:3b) | **v2** (기본) / **v9-ollama-place** (opt-in 시, 2026-05-09 현재 운영) | 기본: 3단계 globalVariantId / opt-in: 2단계 per-model override | `USE_V2_PROMPT=true` 강제 (기본) 또는 `OLLAMA_PROMPT_VARIANT=v9-ollama-place` (2026-05-09 현재) | 운영 제외 (클라우드 전용 대전) | passthrough (운영) | 로컬 전용, v4 적용 대상 아님 (소형 모델). joker-hinter/pair-warmup 은 토큰 예산 위험으로 **비권장** (ADR 44 §6.2). **v7 (2026-04-22)** → **v8 (2026-05-01)** → **v9 (2026-05-09)** 순차 전환. v8은 place rate 15.8%. v9는 5전략 추가(findNewRackSets/findAllExtensions/findRunSplits/findJokerExchange/DFS 최적 초기 등록) + 조커 점수 정확 계산 — 목표 30%+. GPT/Claude/DeepSeek 무영향 |
...
> `src/ai-adapter/src/prompt/registry/variants/` 에서 `PromptRegistry.registerBuiltinVariants()` 가 부팅 시 등록하는 10개.
...
| `v7-ollama-meld` | `variants/v7-ollama-meld.variant.ts` | 1.0.0 | ollama | **true** | 아님 (2026-04-22 도입) | **qwen2.5:3b 전용 하드코딩 프롬프트** — 4-step 절차(Group → Run → Combine → Draw) + hand-holding few-shot 6개 + 색/숫자 힌트. Sprint 7 hotfix: place rate 0% → 목표 >=20%. OLLAMA_PROMPT_VARIANT=v7-ollama-meld opt-in 시에만 활성화. **2026-05-01 이후 v8으로 대체 권장** |
| `v8-ollama-place` | `variants/v8-ollama-place.variant.ts` | 1.0.0 | ollama | **true** | 아님 (2026-05-01 도입) | **qwen2.5:3b 전용 사전 계산 프롬프트** — 빌더(TypeScript)가 `findMeldFor30()` 알고리즘으로 유효 멜드 사전 계산 후 "Output this JSON exactly" 박제. 모델은 복사만. place rate 15.8% (3/19 AI 턴), INVALID_MOVE 0회. 토큰 ~355 (v7 대비 1/6). OLLAMA_PROMPT_VARIANT=v8-ollama-place opt-in 시에만 활성화. **2026-05-01 현재 운영 중** |
| `v9-ollama-place` | `variants/v9-ollama-place.variant.ts` | 1.0.0 | ollama | **true** | 아님 (2026-05-09 도입) | **qwen2.5:3b 전용 개선 알고리즘** — v8 박제 철학 유지 + 5개 전략 추가: findNewRackSets(랙 전용 신규 세트), findAllExtensions(다중 그룹 동시 확장), findRunSplits(6장+ 런 분할, V-13b), findJokerExchange(조커 교체, V-13e), DFS 최적 초기 등록. 조커 점수 정확 계산(30pt 고정 → 대체 위치 숫자값). 목표 place rate 30%+. INVALID_MOVE 0회 유지. **2026-05-09 현재 운영 중** |
...
| 2026-05-01 | §3 표 A 에 `v8-ollama-place` 행 추가 (등록 수 9→10), §2 표 B ollama 행을 v8 현행 운영으로 갱신 | ai-engineer | 2026-05-01 실측: v7도 place rate 0% — 근본 원인은 3B 모델이 멜드 조합 추론 자체를 수행 못 함. v8 전략: TypeScript `findMeldFor30()` 사전 계산 + "Output this JSON exactly" 박제 → place rate 15.8% (3/19 AI 턴), INVALID_MOVE 0회, 토큰 ~355(v7 대비 1/6). `OLLAMA_PROMPT_VARIANT=v8-ollama-place`. 상세: `docs/04-testing/71-v8-ollama-place-experiment-2026-05-01.md` |
