...
- **SSOT 참조**: `55-game-rules-enumeration.md` (활성 77 + 결번 1 = 명목 78), `57-game-rule-visual-language.md` (시각 언어), `60-ui-feature-spec.md` (기능 스펙 F-NN 25개)
...
게임룰 활성 77개(V-* 26 / UR-* 39 / D-* 12, 결번 UR-39 1) 가 위반 또는 적용될 때 사용자에게 전달하는 UX 피드백의 **단일 기준**을 정의한다.
...
| V-22 | 단독 활성 플레이어 자동 승리 (신설 2026-05-06) | GRP-HL (오버레이) | F-16 | 기권(LEAVE/DISCONNECT_TIMEOUT/ABSENT_3_TURNS/AI_FORCE_DRAW_LIMIT)으로 활성 플레이어 1명 이하 시 GAME_OVER (endType: "FORFEIT") 브로드캐스트. UR-27 오버레이 (reason=FORFEIT 카피 분기) + UR-28 ELO 변동. 기권 사전 안내: PLAYER_FORFEITED 수신 시 INFO-T "{displayName}님이 게임에서 나갔습니다" |
...
| v1.2 | 2026-05-06 | game-analyst | SSOT 55 v1.3 동기화. §4.1 V-22 (단독 활성 플레이어 자동 승리) 추가. 기권 4종 트리거(LEAVE/DISCONNECT_TIMEOUT/ABSENT_3_TURNS/AI_FORCE_DRAW_LIMIT)별 UX 매핑 포함. 룰 카운트 활성 76 → 77 (V-* 25 → 26). 명목 77 → 78. |
