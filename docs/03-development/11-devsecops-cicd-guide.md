...
> 버전: v2.0
> 작성일: 2026-03-21
> 최종 수정: 2026-03-21 (Sprint 2 반영 — 13 Jobs GREEN)
...
build stage는 4개 job이 **병렬 실행**된다. `.build-common` 앵커로 공통 설정을 재사용하며, **main 브랜치에서만 실행**된다.
...
> Sprint 2에서 신규 추가된 job. 관리자 대시보드(port 3001)를 빌드하고 레지스트리에 등록한다.
...
| 의존 job | `build-game-server`, `build-ai-adapter`, `build-frontend`, `build-admin` |
...
# admin 이미지 태그 업데이트 (Sprint 2 추가)
sed -i "s|registry.gitlab.com/k82022603/rummiarena/admin:.*|registry.gitlab.com/k82022603/rummiarena/admin:$CI_COMMIT_SHA|g" \
    helm/environments/dev-values.yaml
```
